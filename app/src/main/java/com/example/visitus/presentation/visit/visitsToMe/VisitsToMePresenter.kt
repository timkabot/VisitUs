...
@InjectViewState
class VisitsToMePresenter @Inject constructor(private val visitsInteractor: VisitsInteractor) :
    MvpPresenter<IVisitsListView>() {
    val disposables = CompositeDisposable()

    fun getVisitsToMe() {
        visitsInteractor.getVisitsToMe().observeOn(AndroidSchedulers.mainThread())
            ...
            .subscribe(
                ...
                {
                    Timber.d("Error getting visits to me $it")
                }
            )
            .addTo(disposables)
    }
